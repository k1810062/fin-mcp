...
所有数据来自 MCP Server `fin-data`（注册在 `.mcp.json` 中）。数据库自动创建，无需手动初始化。
...
→ Mode A（静默入库，后台运行）
  执行顺序（同一时间段）：
    ...
    2. fetch.market_data(asset_type="all", mode="batch", ...) — 股票 + ETF 行情
       拉取完成后自动回填 pre_close/pct_chg/change + 权重计算
  最终建议再跑一次维护工具：
    3. maintenance.backfill_changes — 补全可能遗漏的变更记录
    4. maintenance.backfill_weights — 确保所有成分股权重已计算
...
批量更新行情（股票 + ETF 一次性）:
  ...
  → 返回 summary 含 etf_rows, stock_rows, backfill_fixed, weights_calculated

批量更新 PCF（多日）:
  fetch.pcf_components(mode="batch", trade_date="2026-04-20", end_date="2026-04-27")
  → 返回 summary 含 total_days, total_changes
...
数据库统计:
  maintenance.stats()
回填缺失的衍生字段（pre_close/pct_chg/change）:
  maintenance.backfill_quotes()
补全成分股变更记录:
  maintenance.backfill_changes()
计算成分股权重:
  maintenance.backfill_weights()
```
...
每次展示成分股时标注数据来源（pcf/manual）和权重。如果有变更检测记录，用 query.changes 查看历史。
